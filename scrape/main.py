...
def Find_Jobs_In_TopCV(job):
    totalJobs.clear()
    search = job.strip().replace(" ", "-").lower()
...
def Write_To_File(job):
    with open(f'scrape/jobs/{job}.txt', 'w', encoding='utf-8') as f:
...
if __name__ == '__main__':
    while True:
        ...
        time_wait = 24
        print(f"Waiting {time_wait} hours to scrape again")
